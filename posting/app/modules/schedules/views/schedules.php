        ...
        <div class="col-md-12">
            <div class="sc-head">
                <div class="item sc-processing <?=!get("t")?"active":""?>">
                    <a href="<?=cn(segment(1)."/post/schedules/{$date}")?>" class="name"><?=lang('Queue')?> <span><?=($count_status && isset($count_status[1]))?$count_status[1]:0?></span></a>
                </div>
                <div class="item sc-published <?=get("t")==2?"active":""?>">
                    <a href="<?=cn(segment(1)."/post/schedules/{$date}?t=2")?>" class="name"><?=lang('Published')?> <span><?=($count_status && isset($count_status[2]))?$count_status[2]:0?></span></a>
                </div>
                <div class="item sc-unpublished <?=get("t")==3?"active":""?>">
                    <a href="<?=cn(segment(1)."/post/schedules/{$date}?t=3")?>" class="name"><?=lang('Unpublished')?> <span><?=($count_status && isset($count_status[3]))?$count_status[3]:0?></span></a>
                </div>
            </div>
